...
@include('others.sidebar')  
        ...
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="row">
                <div class="col-sm-8"><A href="{{route('admin.exportversions')}}"><button type="button" class="btn btn-primary">Download As Excel</button></A></div>
                <div class="col-sm-4">
  ...
  <thead>
    <tr>
      <th scope="col">Version Num</th>
      <th scope="col">Version Name</th>
      <th scope="col">Book Upload</th>
      <th scope="col">Verses Uploaded</th>
  ...
  <tbody class="bodyclass" style="font-size:12px;">
   ...
   <?php $bookstatus=App\Http\Controllers\Admin\AdminController::Bookcount($row['id']); ?>
   <?php $versesstatus=App\Http\Controllers\Admin\AdminController::Versecount($row['id']); ?>
     
      <tr>
      <td scope="col">{{ $row['version_num'] }}</td>
      <td scope="col">{{ $row['version_name'] }}</td>
      <td scope="col">@if($bookstatus > 0) Uploaded @else Not Uploaded @endif</td>
      <td scope="col">@if($versesstatus > 0) Uploaded @else Not Uploaded @endif</td>
      <?php if($bookstatus > 0 && $versesstatus > 0){ ?>
      <td scope="col"><A href="#" onClick="functionstatuschange(<?php echo $row['id']; ?>,<?php echo $row['status']; ?>)">@if($row['status'] == 1) Active @else In Active @endif</A></td>
      <?php } else { ?>
        <td scope="col">@if($row['status'] == 1) Active @else In Active @endif</td>
        <?php } ?>
      <td scope="col" ><i class="bi bi-pen forpointer"  onClick="editfunction({{$row['id']}})" style="font-size:15px;color:green;margin-left:10px"></i><i class="bi bi-trash forpointer" onClick="deletefunction(<?php echo $row['id']; ?>)" style="color:red;font-size:15px;margin-left:10px;"></i>
</td>
    </tr>
      @endforeach
   @else 
  ...
  <script>
    if('<?php echo count($versions); ?>' == 0 && '<?php echo $versionscount > 0; ?>')
    {
      window.location.href = "{{url('admin/ManageVersion')}}" + "/<?php echo $pagenumber-1; ?>";
    }
    function functionstatuschange(id,status)
    {
      
      $.post("{{ url('admin/StatusChange') }}",{id:id,status:status, _method: 'POST', _token: '{{ csrf_token() }}'})
            .done(function (response) {// Get select
              console.log(response);
              if(response.status == true)
              {
                toastr.error(response.message);
                window.location.href = "{{url('admin/ManageVersion')}}" + "/<?php echo $pagenumber; ?>";
              }
              else
              {
                toastr.error(response.message);
...
{
  window.location.href = "{{url('admin/ManageVersion')}}" + "/" + $(this).val();
});
